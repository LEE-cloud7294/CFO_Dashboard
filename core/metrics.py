...
def calc_kpi(df: pd.DataFrame) -> dict:
    ...
    # 5xx+8xx 중 원재료·부재료·자산처분 버킷 제외
    # - 원재료·부재료: Code 153/162 기준으로 별도 계산
    # - 자산처분: 일회성 비용 → 영업이익 아래 별도 표시
    ...
    # 자산처분 손실 (마이티매매·세척기매각 등 일회성) — 영업이익 아래 표시
    자산처분손실 = cost_by_bucket.get("자산처분", 0)
