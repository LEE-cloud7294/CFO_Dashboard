...
def cost_bucket(code, name, note: str = "") -> Optional[str]:
    ...
    # ── 환경 (오니/슬러지 폐기물 위탁처리 — 적요 우선 판단) ─────────────────
    # 유리 가공 슬러지(오니)를 외부 업체에 위탁처리하는 환경 비용
    if any(k in note for k in ["오니", "슬러지"]):
        return "환경"

    # ── 인건비 (실 급여만 — 직접 지급 임금) ─────────────────────────────────
    if any(k in n for k in ["급여", "임금", "상여", "잡급"]):
        return "인건비"
    if "대광용역" in n:
        return "인건비"
    # 경상연구개발비 → 연구원 급여 지급 형태 → 인건비
    if "경상연구개발" in n:
        return "인건비"

    # ── 퇴직급여 (법정 적립 — 퇴직연금·퇴직금) ─────────────────────────────
    if any(k in n for k in ["퇴직", "DC형"]):
        return "퇴직급여"

    # ── 4대보험 (사용자 법정 부담분) ───────────────────────────────────────
    # 건강보험료·국민연금·고용보험·산재보험 별도 계정
    if any(k in n for k in ["건강보험료", "국민연금", "고용보험", "산재보험"]):
        return "4대보험"
    # 복리후생비 계정 중 보험료 납부 적요 → 4대보험으로 분류
    if "복리후생" in n:
    ...
    if "지급수수료" in n:
        if any(k in note for k in ["조정료", "기장료", "수수료", "월정료", "연회비", "추심"]):
            return "수수료"
        if any(k in note for k in ["오니", "슬러지", "방제"]):
            return "환경"
        if any(k in note for k in ["소방", "안전", "보일러", "방화"]):
            return "안전관리비"
        if any(k in note for k in ["하치장", "중개", "운반", "물류"]):
            return "물류·차량"
        # 시험원·인증·검사 → 연구개발 (한국산업기술시험원 등 품질/성적 시험)
        if any(k in note for k in ["시험", "인증", "심사", "연구", "검사"]):
            return "연구개발"
        return "수수료"
    ...
    # ── 임차료 (공장·사무실 임차 — 세금과공과에서 분리) ─────────────────────
    if any(k in n for k in ["임차료", "임차"]):
        return "임차료"
...
def 보통예금_이자이체(df: pd.DataFrame) -> float:
    """이자비용 = 931(이자비용) 계정 차변 합계.

    과거 "같은 전표의 103 대변 전체 합산" 방식은 위하고가 하루~한달치 거래를
    하나의 전표번호로 묶어 기장하는 특성상, 부가세납부·미지급금반제 등 무관한
    거래까지 통째로 합산되어 실제값(연 2~3억대)의 수십 배로 부풀려지는 버그가
    있었음(2025년 예: 75.7억 표시 vs 실제 931 기장액 2.36억).
    또한 연간 집계 시 전표번호가 매월 00001부터 재시작해 다른 달의 전표끼리
    잘못 묶이는 문제도 있었음. 931 계정 자체는 매월 누락 없이 정상 기장되어
    있음을 확인했으므로 직접 합산이 가장 정확함.
    """
